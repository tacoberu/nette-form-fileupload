 ...
 */
function initMultiFileAjaxUpload(container) {
    const fileInput = container.querySelector('input[type="file"]');
    const preloadButton = container.querySelector('input[name$="[preload]"]');
 ...
 */
async function uploadFileTo(file, url, container) {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
 ...
 */
async function uploadFileInChunks(file, url, container, chunkSize) {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
...
function appendProgressRow(container, name) {
    const uploadRow = container.querySelector('.taco-filecontrol-upload');
 ...
 */
function appendFileRow(container, serialized, name, transaction, preview = '') {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
    if (transactionInput) {
 ...
 */
function appendErrorRow(container, message) {
    const uploadRow = container.querySelector('.taco-filecontrol-upload');
    const row = document.createElement('div');
 ...
 */
function initFileAjaxUpload(container) {
    const fileInput = container.querySelector('input[type="file"]');
    const uploadUrl = container.dataset.uploadUrl;
    const chunkSize = parseInt(container.dataset.chunkSize ?? '0', 10);
    if (!fileInput || !uploadUrl) {
...
async function uploadSingleFileTo(file, url, container) {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
    const data = new FormData();
    data.append('file', file);
    if (transactionInput?.value) {
...
async function uploadSingleFileInChunks(file, url, container, chunkSize) {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
    const chunkId = generateId();
    const chunkTotal = Math.ceil(file.size / chunkSize);
...
function clearSingleFileRow(container) {
...
function setSingleFileRow(container, serialized, name, transaction, preview = '') {
    const transactionInput = container.querySelector('input[name$="[transaction]"]');
    if (transactionInput) {
        transactionInput.value = String(transaction);
    }
...
function appendSingleProgressRow(container, name) {
    const fileInput = container.querySelector('input[type="file"]');
    const row = document.createElement('div');
    row.className = 'taco-filecontrol-row taco-filecontrol-uploading';
...
function appendSingleErrorRow(container, message) {
    const fileInput = container.querySelector('input[type="file"]');
    const row = document.createElement('div');
    row.className = 'taco-filecontrol-row taco-filecontrol-error';
    row.dataset.ajaxInjected = '';
